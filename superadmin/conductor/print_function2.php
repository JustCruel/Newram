...
function printReceipt($fromRoute, $toRoute, $fareType, $totalFare, $conductorName, $busNumber, $transactionNumber, $distance, $driverName, $paymentMethod)
{
    try {
        ...
        function printAlignedLabelValue($printer, $label, $value, $labelWidth = 10, $lineWidth = 32)
        {
            ...
            // Ensure the value fits within the remaining space
            $formattedValue = str_pad($value, $remainingSpace, ' ', STR_PAD_RIGHT);

            // Construct and print the line
            $printer->text($formattedLabel . $formattedValue . "\n");
        }
        ...
        printAlignedLabelValue($printer, "DATE      :  ", $currentDate);
        printAlignedLabelValue($printer, "TIME      :  ", $currentTime);
        printAlignedLabelValue($printer, "FROM      :  ", htmlspecialchars($fromRoute['route_name'] ?? 'Unknown'));
        printAlignedLabelValue($printer, "TO        :  ", htmlspecialchars($toRoute['route_name'] ?? 'Unknown'));
        printAlignedLabelValue($printer, "DISTANCE  :  ", $distance . "KM");
        printAlignedLabelValue($printer, "Driver    :  ", htmlspecialchars($driverName));
        printAlignedLabelValue($printer, "Conductor :  ", htmlspecialchars($conductorName));
        printAlignedLabelValue($printer, "TYPE      :  ", $paymentMethod);
