...
$stmt = $conn->prepare("SELECT 
    (SELECT SUM(amount) FROM transactions WHERE bus_number = ? AND conductor_id = ? AND DATE(transaction_date) = CURDATE()) AS total_load,
    (SELECT SUM(fare) FROM passenger_logs WHERE bus_number = ? AND conductor_name = ? AND DATE(timestamp) = CURDATE()) AS total_fare
FROM DUAL;");
