...
<body>
    ...
    <div class="main-content">
        ...
        <div class="mt-4">
            <h3>Total Load:
                ₱<?php echo number_format($currentDay !== null ? $selectedDayRevenue : array_sum($dailyRevenue), 2); ?>
            </h3>
        </div>

        <!-- Chart Container -->
        <canvas id="revenueChart" class="mt-4" width="400" height="200"></canvas>
    </div>
    ...
    <script>
        ...
        // Get the daily revenues and days in the month
        const dailyRevenue = <?php echo json_encode(array_values($dailyRevenue)); ?>;
        const daysInMonth = <?php echo json_encode(range(1, date('t', strtotime("$currentYear-$currentMonth-01")))); ?>;

        // Create the line chart
        const chart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: daysInMonth, // X-axis: Days of the month
                datasets: [{
                    label: 'Daily Load Transaction (₱)',
                    data: dailyRevenue, // Y-axis: Daily revenues
        ...
        document.getElementById('generatePdfBtn').addEventListener('click', function () {
            ...
            url.searchParams.set('generate_pdf', '1'); // Append parameter to URL
            window.open(url.toString(), '_blank'); // Open PDF in a new tab
        });

        // Toggle day selection based on checkbox
